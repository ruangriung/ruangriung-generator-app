  ...
  return (
    <div className="bg-white pb-16 pt-12 text-slate-900">
      ...
      {activeImage ? (
        ...
        >
          <div
            className="relative max-h-full max-w-3xl overflow-hidden rounded-3xl bg-white shadow-2xl"
            onClick={(event) => event.stopPropagation()}
          >
            <button
              type="button"
              onClick={() => setActiveImage(null)}
              className="absolute right-4 top-4 inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow-lg transition hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"
            >
              <span className="sr-only">Tutup</span>
              ×
            </button>
            <div className="relative max-h-[80vh] w-[min(90vw,960px)]">
              <Image
                src={activeImage}
                alt="Produk UMKM"
                fill
